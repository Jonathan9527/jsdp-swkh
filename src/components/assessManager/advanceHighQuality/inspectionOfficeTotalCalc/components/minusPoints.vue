<template>
  <div class="assessIssue" id="minpointsClassh" v-show="minpoinshow">
        ...
        <el-table-column label="责任单位" prop="flag">
          <template slot-scope="scope">
            <template v-for="item in scope.row.flag">
              <div class="colorfont tableclicksh" @click="dutySuggest(scope.row)">{{item}}</div>
            </template>
          </template>
        </el-table-column>
        <el-table-column label="审核意见" prop="score_opinion"></el-table-column>
        <el-table-column label="审核分" prop="score" width="80"></el-table-column>
        <el-table-column label="补充材料" prop="score_files">
          <template slot-scope="scope">
            <template v-for="item in scope.row.scorefiles">
              <div>
                <a :href="item.href" target="_blank">{{item.name}}</a>
              </div>
            </template>
