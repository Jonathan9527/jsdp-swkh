  ...
  <div class="assessIssue" id = 'inspectocalcClassh' style='margin-top:-7px;'>
    <div v-show="collectMoudle">
      <el-form class="issueTop">
        ...
        <div class="unitList">
          <p>{{selectNain}}年度设区市高质量发展考核指标汇总情况</p>
        </div>
      </el-form>
      <div>
        <div class="tableboxs">
         ...
         <div class="scoringRule">
          ...
          <el-table :data="assessmentScore" id="banzishuju" border style="width: 100%" stripe  >
            ...
            <el-table-column prop="add_minus_score" label="加减分分值">
              <template slot-scope="scope">
                <el-button type="text" size="small" style="background: none;color: #0841B4;" @click="minpoints(scope.row)">
                  <span class="tableclicksh" style="border-bottom: 1px solid #0841B4;">{{scope.row.add_minus_score}}</span>
                </el-button>
              </template>
            </el-table-column>
...
<script>
  ...
  import minusPoints from "./components/minusPoints.vue" //加减分
  ...
  export default {
    ...
    methods: {
      ...
      getData(val) {
        ...
        this.search(params)
        this.huizongshijian(params)
        console.log(params);
        
      },
      search(params) {
        this.$http.post(globalConfig.server2 + 'tuijinHuizong/selectTuijinHuizong', params).then(res => {
      ...
      //查询调上次汇总时间
      huizongshijian(params) {
        this.$http.post(globalConfig.server2 + 'tuijinHuizong/selectTuijinHuizongTime', params).then(res => {
          if (res.data.response_code == "0") {
      ...
      huizongs() {
       ...
       console.log(this.searchParams);
        ...
        this.$http.post(globalConfig.server2 + 'tuijinHuizong/tuijinHuizongCount', params).then(res => {
          if (res.data.response_code == "0") {
            this.createtime = res.data.response_body.createtime
            this.huizongshijian(params)
            this.$message({
      ...
      minpoints(row) {
        this.clickrow = row
        this.clickrow.niandu2 = this.selectNain
