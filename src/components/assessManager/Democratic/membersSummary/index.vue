  ...
  <div id="minzhubanzihyg" style="margin-top:-9px;">
    <div class="title1">
      ...
      <el-select style="width: 100px;" v-model="city" size="medium" @change="search">
        <el-option v-for="item in District" :key="item.city" :label="item.mainValue" :value="item.mainValue">
        </el-option>
      </el-select>-->
    ...
    <div class="title2">
      <p>省直机关领导班子成员民主测评得分 <br>
      <span style="font-size:20px">（{{form.deptName}}）</span></p>
      <!-- <div class="change">
        <span>结果显示：</span>
    ...
    exportExcel() {
      ...
      try {
        FileSaver.saveAs(
          ...
          this.form.niandu + this.form.deptName + "领导班子成员民主测评表.xlsx"
        );
      } catch (e) {
      ...
      this.$http
        ...
        .then(res => {
          if (res.data.response_code === "0") {
            ...
            fd.append("modelType", "省直机关班子成员民主测评表");
            fd.append("type", Number(type) + 1);
            this.$http
              .post(
