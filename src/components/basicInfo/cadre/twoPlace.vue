    ...
    <el-container>
      <el-aside width="337px">
        ...
        <el-scrollbar class="content_scroll">
          <div class="btnGroup">
            <div class="btns">
              <el-button type="primary" class="bluebtnclass" size="medium" @click="addAndEditDepart('add')" :disabled="editDepartStatus">&nbsp;新增</el-button>
      ...
      <el-main>
        ...
        <el-form :inline="true" onsubmit="return false" class="clearfix demo-form-inline">
          ...
          <div style="display: inline-block;">
            <el-form-item style="margin-right: 0">
              ...
              <el-button type="primary" size="medium" @click="linkUserInformDialog" class="bluebtnclass">&nbsp;关联干部人员</el-button>
            </el-form-item>
          </div>
        </el-form>
        <el-table ref="multipleTable" :data="userTableData" tooltip-effect="dark" style="width: 100%;margin-top:10px"
          ...
          <el-table-column label="姓名" prop="peopleName" width="80">
          </el-table-column>
          <el-table-column label="职务" prop="dutyName">
          </el-table-column>
          <el-table-column label="职级" prop="peopleDutyLev" >
          </el-table-column>
          <el-table-column label="职务标签" prop="dutyLabel" width="110">
          </el-table-column>
          <el-table-column label="性别" prop="peopleSex" width="70">
          </el-table-column>
          <el-table-column label="民族" prop="peopleNation" width="80">
          </el-table-column>
          <el-table-column label="出生年月" prop="peopleBirthDate" width="110">
          </el-table-column>
          <el-table-column label="入党时间" prop="peoplePartyDate" width="110">
          </el-table-column>
        </el-table>
        <!-- 分页 -->
        <div class="paging">
  ...
  export default {
    ...
    data() {
      return {
        ...
        editDepartStatus: true, //部门编辑新增按钮状态
        ...
        // 按钮状态
        editBt: true,
        ridOptions: [], //添加弹窗 用户角色
        departAll: [],
        departLeaf: [],
        
        
        usersortData: [],
        departfuzerenDialogVisible:false, //负责人弹窗
        departfuzeren:{},
    ...
    methods: {
      ...
      addAndEditUserClick(type,val){
        if(type === 'add'){
          ...
          this.addAndEditUserForm.name = val.peopleName
          this.addAndEditUserForm.dutyLev = val.peopleDutyLev
          this.addAndEditUserForm.sex = val.peopleSex
          this.addAndEditUserForm.nation = val.peopleNation
          this.addAndEditUserForm.birthDate = val.peopleBirthDate
          this.addAndEditUserForm.partyDate = val.peoplePartyDate
          this.addAndEditUserForm.mainUnitmainFlag = val.mainFlag==0?'否':'是'

          this.addAndEditUserForm.mainUnitdepartmentId = val.departmentId
          this.addAndEditUserForm.mainUnitdutyLabel = val.dutyLabel
          this.addAndEditUserForm.mainUnitdutyName = val.dutyName
          this.addAndEditUserForm.mainUnit = '是'

          this.$http.post(globalConfig.server1 + 'People2/select-people-ById', {"prdId":val.id}).then(res => {
              if(res.data.code === 200){
                this.checkmainDepart = res.data.data.departmentId
        ...
        this.$http.post(globalConfig.server1 + "People2/select-peopleByPages", params).then(res => {
            if(res.data.code === 200){
              res.data.data.list.forEach(item=>{
                item.peopleBirthDate = item.peopleBirthDate?this.$tools.formatTime(item.peopleBirthDate,'YYYY-MM-DD'):""
              })
