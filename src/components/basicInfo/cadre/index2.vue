        ...
        <el-form :inline="true" onsubmit="return false" class="clearfix demo-form-inline">
          ...
          <div style="display: inline-block;">
            <el-form-item style="margin-right: 0">
              ...
              <el-button type="primary" size="medium" @click="linkUserInformDialog" class="bluebtnclass" :disabled="!clicknode.id || firstUnit.indexOf(clicknode.id) != -1">&nbsp;关联干部人员</el-button>
            </el-form-item>
          </div>
        </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getdepartmentList(){
        this.$http.post(globalConfig.server1 + 'department2/select-MainDepartment',{"niandu":this.departParams.niandu,"type":"gb2_dept_"}).then(res => {
        ...
        this.$http.post(globalConfig.server1 + 'department2/select-department-tree', params).then(res => {
          if (res.data.code === 200) {
            if(res.data.data.id){
              ...
              this.firstUnit = [res.data.data.id,...res.data.data.children.map(el=>el.id)]
            }
            else
              this.departmentTree = []
          } else {
       ...
       */
      searchAllPeople(){
        this.$http.post(globalConfig.server1 + "People2/query_PeopleByDepartment", {type:"gb2",niandu:this.params.niandu||""}).then(res => {
            if(res.data.code === 200){
